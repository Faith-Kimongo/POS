...
<br>
			...
			<div class="section-title mt-6"  style="margin-top: 10px">
				<h4> <a href="/restaurant/{{Session::get('branch')}}/{{Session::get('hotel_id')}}/{{Session::get('table_no')}}">Home </a> >> <a href="/category/{{$category->category->id}}" >{{ucfirst($category->name)}} </a> >> {{ucfirst($category->name)}}</h4>
